...
test('config.js loads (default config file)', (t) => {
    ...
    if (!defExists) {
        fs.writeFileSync(DEFAULT_CONFIG_FILE, 'PLACEHOLDER')
    }
    if (altExists) {
        fs.renameSync(ALT_CONFIG_FILE, ALT_CONFIG_FILE_RENAMED)
    }
    ...
    if (altExists) {
        fs.renameSync(ALT_CONFIG_FILE_RENAMED, ALT_CONFIG_FILE)
    }
})
...
test('config.js loads (alternate config file)', (t) => {
    ...
    if (defExists) {
        ...
        fs.writeFileSync(
            ALT_CONFIG_FILE,
            JSON.stringify({ config: DEFAULT_CONFIG_FILE })
        )
    }
    ...
    if (defExists) {
        fs.renameSync(DEFAULT_CONFIG_FILE_RENAMED, DEFAULT_CONFIG_FILE)
    }
    if (!altExists) {
        fs.unlinkSync(ALT_CONFIG_FILE)
    }
})
...
test.skip('Verify config.js default config file format', (t) => {
    let defExists = fs.existsSync(DEFAULT_CONFIG_FILE)
    if (!defExists) {
        t.pass('No default config file found')
    }
...
test.skip('Verify config.js custom config format if file exists', (t) => {
    let altExists = fs.existsSync(ALT_CONFIG_FILE)
    if (!altExists) {
        t.pass('No alternate config file found')
    }
