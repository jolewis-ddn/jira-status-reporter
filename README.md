...
- `/groups`: Shows a list of groups.
  ...
  - `?filter=yes` to see the subset (as specified by the userGroups config value)
    - If userGroups array _is set_ in the config, it will be excluded (and the members of the remaining groups will be fetched).
    - If userGroups _is not set_, the full list of groups (without members) will be fetched.
